...
public class UserInfoServiceImpl extends ServiceImpl<UserInfoMapper, UserInfo> implements IUserInfoService {
    ...
    @Resource
    private RedisCache redisCache;//在redis里面去查取用户id
    ...
    public UserInfo getByUserId(Long userId){
        ...
        if (Objects.nonNull(userInfo)){//如果redis里面有用户信息，直接返回
            return userInfo;
        }

        //如果没有，从数据库里面去查取用户信息
        userInfo = lambdaQuery().eq(UserInfo::getUserId, userId).one();
        if (Objects.isNull(userInfo)) {//如果数据库里面也没有用户信息，抛出异常
            throw new HmdpFrameException(BaseCode.USER_NOT_EXIST);
        }
        //不为空的话，将用户信息缓存到redis里面
