...
import { getShopReviews } from '@/api/service'
...
// 跳转笔记评论页
const toComments = () => {
  router.push(`/blogComments/${route.params.id}`)
}

// 关联商户的网友评价（真实探店笔记）
const shopReviews = ref([])
const queryShopReviews = (shopId) => {
  if (!shopId) return
  getShopReviews(shopId)
    .then(({ data }) => {
      shopReviews.value = data || []
    })
    .catch(() => {
      shopReviews.value = []
    })
}
const toReviewerInfo = (userId) => {
  if (userId) router.push(`/InfoOther?id=${userId}`)
}
...
const initData = async () => {
  ...
  try {
    ...
    queryShopReviews(blog.value.shopId)
...
<template>
  <div class="blog-detail">
    ...
    <div class="content">
      ...
      <div class="blog-comments">
        <div class="comments-head">
          <div>网友评价 <span>（{{ shopReviews.length }}）</span></div>
          <div>
            <el-icon><ArrowRight /></el-icon>
          </div>
        </div>
        <div class="comment-list">
          <div class="comment-box" v-for="b in shopReviews" :key="b.id">
            <div class="comment-icon" @click="toReviewerInfo(b.userId)">
              <img
                :src="
            ...
            <div class="comment-info">
              <div class="comment-user" @click="toReviewerInfo(b.userId)">
          ...
          <div v-if="shopReviews.length === 0" class="comment-empty">
            这家店还没有评价，发布探店笔记抢个沙发～
          </div>
        </div>
      </div>
    ...
    <div class="foot">
      ...
      <div class="foot-box">
        <div class="foot-view" @click="toComments">
